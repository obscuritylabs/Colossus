...
/// Credential-free SearXNG connection generated by an app-managed runtime.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ManagedSearchConfig {
    /// Stable search profile identifier.
    pub profile: String,
    /// Exact SearXNG JSON search endpoint.
    pub endpoint: String,
    /// Per-request transport timeout.
    pub timeout_ms: u64,
}
...
impl ManagedSearchConfig {
    /// Validate a bounded credential-free SearXNG profile.
    pub fn validate(&self) -> Result<(), ProtocolError> {
        ...
        if !valid_token(&self.profile)
            ...
            || !endpoint.path().ends_with("/search")
        {
            return Err(ProtocolError::InvalidFrame);
        }
        Ok(())
