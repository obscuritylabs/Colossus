...
use windows_sys::Win32::{
    Foundation::{
        CloseHandle, ERROR_INSUFFICIENT_BUFFER, GetLastError, HANDLE, HANDLE_FLAG_INHERIT, HLOCAL,
        INVALID_HANDLE_VALUE, LocalFree, SetHandleInformation, WAIT_FAILED, WAIT_OBJECT_0,
        WAIT_TIMEOUT,
    },
    NetworkManagement::WindowsFilteringPlatform::{
        ...
        FWP_V4_ADDR_MASK, FWP_VALUE0, FWP_VALUE0_0, FWPM_ACTION0, FWPM_CONDITION_ALE_PACKAGE_ID,
...
fn add_filter(
    ...
    let result = unsafe { FwpmFilterAdd0(engine, &raw const filter, null_mut(), &mut filter_id) };
    if result == 0 {
        Ok(())
    } else {
...
impl AttributeList {
    fn new(count: u32) -> Result<Self, WindowsProcessError> {
        ...
        let result = unsafe { InitializeProcThreadAttributeList(null_mut(), count, 0, &mut bytes) };
    ...
    fn set(
        ...
        if unsafe {
            UpdateProcThreadAttribute(self.raw(), 0, attribute, value, bytes, null_mut(), null())
        } == 0
        {
            Err(last_error(operation))
        } else {
