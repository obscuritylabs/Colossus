...
struct VolumeRecord {
    identity: NfsVolumeIdentity,
    device_major: u32,
    device_minor: u32,
}

fn parse_record(line: &str, line_number: usize) -> io::Result<VolumeRecord> {
    ...
    let fscache = fields
        .next()
        .ok_or_else(|| malformed_table(Some(line_number)))?;
    if fields.next().is_some() || !matches!(fscache, "yes" | "no") {
        return Err(malformed_table(Some(line_number)));
    }

    let nfs_version = match version {
        "v2" => 2,
        "v3" => 3,
        "v4" => 4,
    ...
    let server_address =
        parse_server_address(server).ok_or_else(|| malformed_table(Some(line_number)))?;
    let server_port = parse_canonical_hex(port, 4)
    ...
    let (fsid_major, fsid_minor) =
        parse_fsid(fsid).ok_or_else(|| malformed_table(Some(line_number)))?;
