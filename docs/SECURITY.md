...
for hard active-process, per-process memory, and aggregate job-memory messages. The
...
Windows Job Object effects require at least a 10-second total timeout. The helper receives
the execution budget minus a 7-second reserve so termination, output capture, temporary
