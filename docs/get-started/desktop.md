...
From the `v0.10.2-preview.3` Developer Preview, download
`Colossus-Desktop-DEVELOPER-PREVIEW-v0.10.2-preview.3-aarch64-apple-darwin.zip`
and its adjacent `.sha256` file. Keep both files together and verify the archive before
opening it:

```bash
shasum -a 256 -c \
  Colossus-Desktop-DEVELOPER-PREVIEW-v0.10.2-preview.3-aarch64-apple-darwin.zip.sha256
```
