...
lockfile to audit, test, and build the renderer. It does not allocate macOS or Windows
runners. The aggregate gate accepts a skipped job only when the classifier explicitly
marked that job unnecessary; the stable seven-argument gate contract remains unchanged.
...
acceptance work. The existing macOS native job also formats, lints, tests, and builds the
standalone Tauri application, while supply-chain acceptance audits its independent lockfile.
