...
without embedded fallback. A busy Windows pipe receives bounded connection and
authenticated-handshake retries and also never falls through to embedded mode. If no
worker endpoint exists, the same runtime embeds safely.
