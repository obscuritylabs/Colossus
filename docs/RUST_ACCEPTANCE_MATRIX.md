...
| FLOW-01 | Passing P1 + triggers | strict YAML/hash trust; bounded control flow; waits; idempotent retry; compensation; subworkflows; cancellation; fixed-cadence schedule misfire/trust/restart reconstruction; atomic deterministic schedule dispatch under process kill; HMAC webhook authentication/replay/size/trust rejection; atomic accepted-delivery/run queueing; worker/embedded routing | repository events and subscriptions |
...
| DIST-01 | Passing P2 foundation | clean-prefix native installers; signed bundle trust/tamper/reproducibility tests; installed echo and encrypted audit smoke without Cargo, credentials, or network; green six-target `v0.7.0` release run | future distribution adapters and signed offline collections |
