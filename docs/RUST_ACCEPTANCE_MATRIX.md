...
| TOOL-01 | Passing P0 catalog foundation | strict configured catalog; schema-before-policy validation; complete offline filesystem, Git, shell, user prompt, work, plan/goal, patch, repository, subagent, discovery, trace, context, skill/resource, and echo tools; configured MCP calls; exact-origin `web.fetch`/`docs.fetch`; permit-bound repository/patch/context/export effects; metadata-only trace views; interactive-only `user.ask`; event-sourced policy-authorized REPL preferences plus semantic work/context/provider-event rendering with worker parity; traversal, control-state, symlink, binary, ambiguity, scope, approval, and post-release tests | Remaining tool-family semantic renderers, recoverability/elapsed activity presentation, and broader live endpoint/tool-use acceptance |
