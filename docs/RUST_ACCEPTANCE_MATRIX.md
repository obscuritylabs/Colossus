...
| STORE-01 | Passing embedded and memory-index foundation | split ports; in-memory and redb journal/projection conformance; atomic outbox; exclusive writer lease; restart catch-up; optimistic positions; deterministic rebuild; startup position validation; session/work/memory/workflow reducers; canonical session/message and memory repositories; Tantivy and permit-bound Chroma projection conformance | Research/extension repositories, queued external index/export work, worker IPC, and additional adapter conformance suites |
