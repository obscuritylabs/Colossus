...
pub async fn prompt(parent: tauri::WebviewWindow) -> Result<HostSecret, PromptError> {
    ...
    #[cfg(any(windows, target_os = "macos"))]
    {
        ...
        parent
            .run_on_main_thread(move || {
                #[cfg(windows)]
                crate::windows::open(&native_parent, cancelled, completion);
                #[cfg(target_os = "macos")]
                crate::macos::open(&native_parent, cancelled, completion);
            })
