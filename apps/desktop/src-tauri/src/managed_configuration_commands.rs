...
use crate::{
    ...
    desktop_settings::{
        AccessProfileSetting, DesktopSettings, ExecutionBoundarySetting, SettingsStore,
        delete_provider_secret, store_provider_secret,
    },
    dto::CommandErrorDto,
    managed_configuration::{
        ...
        McpServerSetting, ResolvedSpaceConfiguration, SpaceConfigurationSetting,
        resolve_space_configuration,
    },
...
const LOCKED_INVARIANTS: &[(&str, &str, &str)] = &[
    ...
    (
        "sandbox.filesystem",
        "Desktop filesystem grants",
        "Desktop compiles workspace and credential grants from native user selections.",
    ),
    (
        "sandbox.executables",
        "Desktop executable grants",
        "Desktop compiles exact executable grants from managed integrations.",
    ),
    (
        "sandbox.networkDestinations",
        "Desktop network grants",
        "Desktop compiles exact network destinations from accepted managed resources.",
    ),
];
...
fn apply_space_edit(
    ...
    let mcp_references = request
        .selected_mcp_resource_ids
        .iter()
        .map(|resource_id| {
            let entry = settings
                .global_configuration
    ...
    space
        .configuration
        .catalog_revisions
        .retain(|key, _| !key.starts_with("mcp:"));
    space.configuration.catalog_revisions.extend(mcp_references);
...
async fn confirm_authority_elevation(
    app: &AppHandle,
    ...
    if !access_elevated && !boundary_elevated {
        return Ok(());
    }
    let app = app.clone();
    let approved = tauri::async_runtime::spawn_blocking(move || {
        app.dialog()
            .message(
                "This change increases the Space's authority. Review the Access and Sandbox values before allowing the runtime to restart.",
            )
...
fn bump_global_revision(global: &mut GlobalConfigurationSetting) -> Result<(), CommandErrorDto> {
        ...
        for (key, reference) in &space.configuration.catalog_revisions {
            if key.starts_with("mcp:")
                && settings
                    ...
                    .is_some_and(|revision| mcp_uses_credential(&revision.value, credential_id))
            {
                dependents.insert(format!("Space {} MCP selection", space.display_name));
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::desktop_settings::{WorkspaceProfile, WorkspaceSetting};
